...
export default function Result2() {
  ...
  console.log("123",histories)
  return (
    <div className="p-4">
      <h1 className="bg-black text-white text-center py-6 rounded-lg text-2xl">
        Quiz Review
      </h1>
      {histories?.data?.historyAssignment
  .filter(history => history.userAnswer !== null) // Lọc các mục có userAnswer khác null
  .map((history, historyIndex) => (
    <div key={historyIndex} className="mt-6">
      <ul
        className={`p-4 rounded-lg ${
          history.result
            ? "bg-green-100 border-3 border-green-200"
            : "bg-red-100 border-3 border-red-200"
        }`}
      >
        ...
        <p className="bg-gray-200 p-2 rounded-lg border-2 border-gray-300 mt-2">
          Right answer is:{" "}
          <span className="font-bold">{history.correctAnswer}</span>
        </p>
      </ul>
    </div>
  ))}
