...
export function SidebarItem({ icon, text, active, alert, onClick }) {
    ...
    return (
        ...
        >
            ...
            {!expanded && (
                <div className="absolute left-full whitespace-nowrap rounded-md px-2 py-1 ml-3 bg-indigo-100 text-indigo-800 text-sm invisible opacity-0 -translate-x-3 transition-all duration-300 ease-in-out group-hover:visible group-hover:opacity-100 group-hover:translate-x-0">
                    {text}
                </div>
            )}
        </li>
